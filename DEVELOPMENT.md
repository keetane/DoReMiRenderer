...
`Examples/WebCanvasViewer` is the browser-facing DoReMi Palette example. It
renders a `ScoreWebRenderPlan` generated by the SDK and keeps all notation
geometry in that plan. The toolbar opens exported JSON plans directly and,
when started with `Examples/WebCanvasViewer/server.py`, also uploads
MusicXML/MXL to the loopback server for SDK conversion. Direct MusicXML/MXL
parsing remains a native app or server responsibility. The Web
layout profile uses a fixed 36pt time-signature digit size, a 4pt digit inset
toward the staff centre, and a 90pt grand-staff inter-staff gap without changing
iOS or PDF settings. Palette note colours, clef-aware staff-line colours, and
keyboard selection consume the exported stable note anchors and MIDI values.
The Web palette mirrors iOS `defaultEducationalPalette`: C/C#, D/D#, E, F/F#,
...
and follow moves vertically only on an SDK-defined system transition. The
selected major/minor scale drives staff and keyboard colour membership; a
separate display-transpose switch is intentionally not present. The Web Audio
engine mirrors the iOS `SimpleToneAudioEngine` harmonic waveform, velocity
mapping, low-pitch boost, and 10ms envelope; individual chord pitches retain
their exported written durations. The palette owns the note-colour checkbox,
a circular pitch-colour next-note keyboard marker, and an 8pt black-key edge
band rendered with the same inset model as white keys. The Web toolbar exposes
an editable 50–200% score-zoom field with 10% step controls beside an editable
30–300 BPM control. Each SDK playback event carries its source BPM,
so Web transport scales event intervals and written sound durations from a real
tempo number rather than a percentage without changing layout. Web
plans use system-leading measure numbers, so only the leftmost measure of an
exported grand staff is labelled.
...
- A4 print vertical margins are 36pt at the top and bottom. Print grand-staff
  ...
  is 68pt. The page-fit pass and final relocation use the same value, so a
...
- Latest page-fallback visual QA:
  ...
  upper/lower grand-staff whitespace, 68pt system gaps, stable gaps on pages
...
`ScoreWebLayoutProfile.responsive` is independent from the app's print profile:
it uses a 12pt staff space, 82pt system spacing, no artificial measure gutter,
90pt of grand-staff interline whitespace, and adds 20pt to each horizontal
content margin. It reserves 120pt between the title and the first grand staff,
centers half/whole rests in their measure, and places a whole rest against the
second staff line. It repeats prefixes at wrapped
system starts, keeps dense short-note groups at their rhythmic onset, separates
dynamics/hairpins into below-staff lanes, hides pedal labels, increases
noteheads by 2pt, enlarges time signatures 1.5x while insetting each digit 3pt
toward the staff centre, and reduces title emphasis for scroll reading. All
notation proportions derive from `staffSpace`.
`LayoutOptions.printSystemGap` defaults to the restored 68pt iOS/PDF print
rhythm and is applied only during page pagination, so browser tuning cannot
change an app PDF. Shared notation hardening overlaps each stem slightly into
its notehead frame to avoid a visible Canvas/SMuFL seam; the Web profile adds a
further 2pt attachment inset. Adjacent chord seconds reflect the lower notehead
across the common stem axis without moving the stem, flag, or beam. Closing-repeat
...
The Web palette retains an iOS-compatible educational colour grouping while
respecting score spelling at its boundary: browser note anchors expose both
sounding MIDI and their unaltered diatonic pitch class, so B-flat can retain B
family colour even though its keyboard position is the A-sharp black key.
Coloured staff rules are layered over standard staff ink and below notation;
the browser never shifts notation to obtain this effect. The palette supplies
check-style controls for staff colour, keyboard colour, next-note guide, and
the global pitch-class set, plus a top/bottom key-colour position control.
