...
- `catalog.json` and one SDK-generated Web Render Plan per MXL in
  `Apps/DoReMiPalette/DoReMiPalette/Resources/Samples/`.
- Origin: static plans generated by `Scripts/generate-web-sample-catalog.sh`
  through `DoReMiRendererWebExport`; plans do not include the original MXL
  archives.
- Distribution: static GitHub Pages learning samples, expanded to the complete
  current MXL catalog on 2026-09-02 at the product owner's direction.
- Source: the MXL source set originates from musetrainer/library. Its README
  and GitHub About state "Public domain MusicXML files," but its repository
  root had no `LICENSE`, `COPYING`, or `NOTICE` in the 2026-05-21 audit.
- License position: publishing a derived render plan does not establish a grant
  for the underlying MusicXML transcription, arrangement, fingering, or export
  data. The complete web catalog is not individually cleared for commercial
  distribution. Retain the source attribution and re-audit every selected score
  before commercial distribution or App Store bundle inclusion.
